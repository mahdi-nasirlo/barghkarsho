<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()" :label-sr-only="$isLabelHidden()" :helper-text="$getHelperText()"
    ...
    <div x-data="markdownEditorFormComponent({
        ...
        {{-- <div x-model="state" class="form-textarea w-full" x-data="editorApp()" x-init="init($dispatch)" wire:ignore
            wire:key="ckEditor" x-ref="ckEditor" wire:model.debounce.9999999ms="content"></div> --}}
        ...
        <script>
            const editor ClassicEditor.create(document.querySelector('.editor')
                    ...
                    editor.setData('alksdjflkasjflkdj')
                    /**
        ...
        <script>
            ClassicEditor
                ...
                .catch(error => {
                    ...
                    console.warn('Build id: 2gplqni1iwfb-zhwyx7hr4m15');
                    console.error(error);
                });
        </script>
        {{-- <div class="space-y-2">
                            ...
                            <div class="flex items-stretch space-x-1 rtl:space-x-reverse">
                                @if ($hasToolbarButton('bold'))
                                    <x-forms::markdown-editor.toolbar-button
                                        ...
                                        <md-bold>
                                            <svg @class(['h-4', 'dark:fill-current' => config('forms.dark_mode')]) aria-hidden="true" focusable="false"
                    ...
                    @if ($hasToolbarButton(['edit', 'preview']) && $isEnabled())
                        <div class="flex items-center space-x-4 rtl:space-x-reverse">
                            @if ($hasToolbarButton('edit'))
                                <button type="button" x-on:click.prevent="tab = 'edit'"
                                    x-bind:class="{
                                        'text-gray-400 @if (config('forms.dark_mode')) dark:text-gray-400 @endif': tab !==
                                            'edit'
                                    }"
                            ...
                            @if ($hasToolbarButton('preview'))
                                <button type="button" x-on:click.prevent="tab = 'preview'"
                                    x-bind:class="{
                                        'text-gray-400 @if (config('forms.dark_mode')) dark:text-gray-400 @endif': tab !==
                                            'preview'
                                    }"
            ...
            <div x-show="tab === 'edit'" class="relative w-full h-full" style="min-height: 150px;">
                <file-attachment directory>
                    <textarea {!! $isAutofocused() ? 'autofocus' : null !!} id="{{ $getId() }}" {!! ($placeholder = $getPlaceholder()) ? "placeholder=\"{$placeholder}\"" : null !!} x-model="state"
                        dusk="filament.forms.{{ $getStatePath() }}" x-on:keyup.enter="checkForAutoInsertion"
                        ...
                        "
                        x-ref="textarea" @if (!$isConcealed()) {!! $isRequired() ? 'required' : null !!} @endif @class([
                            ...
                            'dark:caret-white dark:focus:border-primary-600' => config(
                                'forms.dark_mode'
                            ),
                ...
                <div x-ref="overlay" x-html="overlay" style="min-height: 150px;" @class([
